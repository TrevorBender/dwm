...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]);
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? tag_schemes[i] : SchemeTag]);
...
{
	unsigned int n = 0;
	unsigned int ms = LENGTH(monocle_symbols);
	Client *c;
	...
	if (n > 0) /* override layout symbol */
		snprintf(m->ltsymbol, sizeof m->ltsymbol, "%s", monocle_symbols[n > ms ? ms-1 : n-1]);
